    ...
    <script>
        ...
        function login() {
            ...
            if(flag==true&&flag2==true){
                $.ajax({
                    url: '/rest/api/manger/login',
                    dataType: "json",
                    type: 'post',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    data: { email: account,password:pas },
                    success: function (data) {
                        if(data.success==1){
                            console.log("AA")
                            console.log(Cookies.set('admin', account,{ expires: 1 }));
                            //console.log(Cookies.set('shop',account,{expries:1}));
        ...
        function reg(){
            ...
            var pas = document.getElementById('reg_password').value
            var pas2=document.getElementById('reg_check_password').value
            var emailRule = /^\w+((-\w+)|(\.\w+))*\@[A-Za-z0-9]+((\.|-)[A-Za-z0-9]+)*\.[A-Za-z]+$/;
            if(pas==pas2&&account.search(emailRule)!= -1){
                $.ajax({
                    url: '/rest/api/manger/reg',
                    dataType: "json",
                    type: 'post',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                ...
                document.getElementById('warning2').innerHTML='有錯誤!!';
            }
        }
    </script>
    <style>
          ...
          <span aria-hidden="true">&times;</span>
                ...
                <div class="modal-body">
                    <form>
                        <div class="form-group">
                            <label for="exampleInputEmail1">帳號</label>
                            <input type="email" class="form-control" id="reg_account" aria-describedby="emailHelp" placeholder="輸入email">
                        </div>
                        <div class="form-group">
                            <label for="exampleInputPassword1">密碼</label>
                            <input type="password" class="form-control" id="reg_password" placeholder="密碼">
                        </div>
                        <div class="form-group">
                            <label for="exampleInputPassword1">再次確認密碼</label>
                            <input type="password" class="form-control" id="reg_check_password" placeholder="輸入剛剛輸入的密碼">
                        </div>
                        <p class="h6 text-danger" id="warning2"></p>
                        <a class="btn btn-danger text-light" onclick="reg()">註冊</a>
                    </form>
                </div>
            </div>
        </div>
